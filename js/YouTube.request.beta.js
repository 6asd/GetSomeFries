...
const $ = new ENV("🍟 GetSomeFries: ▶️ YouTube v0.1.5(1005) request.beta");
...
$.log(`⚠ FORMAT: ${FORMAT}`, "");
(async () => {
	const { Settings, Caches, Configs } = setENV("GetSomeFries", "YouTube", Database$1);
				...
				default:
					...
					switch (PATH) {
						case "/watch":
							...
							//if ($request.headers["User-Agent"]) $request.headers["User-Agent"] = "Mozilla/5.0 (iPhone; CPU iPhone OS 17_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.4 Mobile/15E148 Safari/604.1";
							//if ($request.headers["user-agent"]) $request.headers["user-agent"] = "Mozilla/5.0 (iPhone; CPU iPhone OS 17_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.4 Mobile/15E148 Safari/604.1";
							body = body.replace('"contextId":"WEB_PLAYER_CONTEXT_CONFIG_ID_KEVLAR_WATCH",','"contextId":"WEB_PLAYER_CONTEXT_CONFIG_ID_KEVLAR_WATCH","controlsType":"3",');
							break;
					}					break;
				case "CONNECT":
				case "TRACE":
